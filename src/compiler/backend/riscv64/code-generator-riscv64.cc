...
void CodeGenerator::BailoutIfDeoptimized() {
  ...
  __ RecordComment(
      "[  Ld(kScratchReg, MemOperand(kJavaScriptCallCodeStartRegister, "
      "offset));");
  ...
  switch (arch_opcode) {
    ...
    case kArchCallJSFunction: {
      ...
      if (FLAG_debug_code) {
        ...
        __ RecordComment(
            "[  Ld(kScratchReg, FieldMemOperand(func, "
      ...
      __ RecordComment(
          "[  Ld(a2, FieldMemOperand(func, JSFunction::kCodeOffset));");
    ...
    case kArchDeoptimize: {
      ...
      __ RecordComment("[  Branch(exit->label());");
    ...
    case kArchStoreWithWriteBarrier: {
      ...
      __ RecordComment("]");
      __ RecordComment("[  Sd(value, MemOperand(kScratchReg));");
    ...
    case kRiscvTruncLD: {
      ...
      if (set_overflow_to_min_i64) {
        __ RecordComment("[  Add64(kScratchReg, i.OutputRegister(), 1);");
...
#define __ tasm->
  ...
  if (instr->arch_opcode() == kRiscvTst) {
    ...
    cc = FlagsConditionToConditionCmp(condition);
    __ RecordComment(
        "[  Branch(tlabel, cc, i.InputRegister(0), Operand(zero_reg));");
...
void CodeGenerator::AssembleReturn(InstructionOperand* additional_pop_count) {
  ...
  if (call_descriptor->IsCFunctionCall()) {
    ...
    if (drop_jsargs) {
      ...
      __ RecordComment(
          "[  Ld(t0, MemOperand(fp, StandardFrameConstants::kArgCOffset));");
  ...
  if (drop_jsargs) {
    ...
    __ RecordComment(
        "[  Add64(t0, t0, Operand(1));  // Also pop the receiver.");
    __ Add64(t0, t0, Operand(1));  // Also pop the receiver.
    __ RecordComment("]");
    if (parameter_slots > 1) {
      Label done;
      __ RecordComment("[  li(kScratchReg, parameter_slots);");
      __ li(kScratchReg, parameter_slots);
      __ RecordComment("]");
      __ RecordComment("[  Branch(&done, ge, t0, Operand(kScratchReg));");
      __ Branch(&done, ge, t0, Operand(kScratchReg));
      __ RecordComment("]");
      __ RecordComment("[  Move(t0, kScratchReg);");
    ...
    __ RecordComment("[  Sll64(t0, t0, kSystemPointerSizeLog2);");
    __ Sll64(t0, t0, kSystemPointerSizeLog2);
    __ RecordComment("]");
    __ RecordComment("[  Add64(sp, sp, t0);");
    __ Add64(sp, sp, t0);
    __ RecordComment("]");
    ...
    __ RecordComment("[  Drop(parameter_slots + additional_count);");
    __ Drop(parameter_slots + additional_count);
    __ RecordComment("]");
  } else {
    Register pop_reg = g.ToRegister(additional_pop_count);
    __ RecordComment("[  Drop(parameter_slots);");
...
void CodeGenerator::FinishCode() {
  __ RecordComment("[  ForceConstantPoolEmissionWithoutJump(); }");
  __ RecordComment("]");
}
